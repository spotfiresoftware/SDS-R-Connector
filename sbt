...
declare -r jit_opts="-XX:ReservedCodeCacheSize=512m -XX:+TieredCompilation"

if [ -f "$HOME/dev/jrebel/jrebel.jar" ];
then
  declare -r default_jvm_opts="-Dfile.encoding=UTF8 -XX:MaxPermSize=1024m -Xms1024m -Xmx2048m -Xss32m $jit_opts $cms_opts -noverify -javaagent:"$HOME/dev/jrebel/jrebel.jar""
else
  declare -r default_jvm_opts="-Dfile.encoding=UTF8 -XX:MaxPermSize=1024m -Xms1024m -Xmx2048m -Xss32m $jit_opts $cms_opts"
fi
